/**
 * Copyright (c) 2023 kprepos. All rights reserved.
 * https://github.com/kprepos/audio2text
 * 
...
export const getFileBlob = async (path: string): Promise<Blob | null> => {
    // SiYuan kernel expects workspace-relative paths, e.g. "/data/assets/xxx.wav"
    ...
    // Important: kernel returns status 202 with JSON body on error (see SiYuan API docs)
    if (response.status !== 200) {
        try {
            const err = await response.json();
            console.error("[getFileBlob] getFile failed", { path, normalizedPath, status: response.status, err });
        } catch (e) {
            console.error("[getFileBlob] getFile failed", { path, normalizedPath, status: response.status, e });
        }
        return null;
    }
